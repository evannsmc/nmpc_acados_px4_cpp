...
This package was created during a PhD at Georgia Tech's FACTSLab as a high-performance C++ counterpart to the Python NMPC baseline used for comparisons against Newton-Raphson Flow controllers. Because it links against the C solver emitted by the Python package's code-generation step, the Python package (`nmpc_acados_px4`) must be present in the same workspace and run first.
